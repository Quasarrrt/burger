...
export const ProfileOrdersPage: React.FC<IProfileOrdersPage> = ({
  onCardOrderClick,
}) => {
  ...
  const { ordersData }: { ordersData: TOrders } = useSelector(
  ...
  React.useEffect(() => {
    ...
    dispatch({
      ...
      wsUrl: `${WS_USER_ORDERS_URL}?token=${token}`,
    });
    return () => {
            ...
            {ordersData?.orders?.map((order: TOrder) => {
              return (
                <CardOrder
                  order={order}
                  key={order._id}
                  onCardOrderClick={onCardOrderClick}
                />
              );
            })}
          </ul>
